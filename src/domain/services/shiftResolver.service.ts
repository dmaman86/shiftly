...
export const shiftResolver = ((fieldMinutes, fieldShiftPercent) => {
  const { min06, min14, min17, min22, fullDay } = fieldMinutes;
  const { hours50, hours20, hours100, hours150, hours200 } = fieldShiftPercent;


  const buildSegmentMap = (specialStart: number): Record<WorkDayType, LabeledSegmentRange[]> => {
    return {
      [WorkDayType.Regular]: [
        {
  ...
  ): LabeledSegmentRange[] => segments.map(({ point, percent, key }) => ({
      point: {
  ...
  return ({
    ...
    const { minutes: startMin } = shift.start;
    const { minutes: endMin } = shift.end;
    ...
    switch (meta.typeDay) {
        case WorkDayType.Regular:
          return handleRegular({ start: startMin, end: endMin }, segmentMap.Regular);
        case WorkDayType.SpecialPartialStart:
          return handlePartialStart({ start: startMin, end: endMin }, segmentMap);
        case WorkDayType.SpecialFull:
          return handleSpecial({ start: startMin, end: endMin }, meta.crossDayContinuation, segmentMap);
        default:
          return [];
    }
  };
})(defaultFieldMinutes, defaultFieldShiftPercent);
